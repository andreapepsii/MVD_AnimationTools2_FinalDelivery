...
void ViewTrack::update(float dt) {
	...
	if (active || start)
	{
		...
		// Apply transforms and reset ratio
		{
			...
			//set target if there's any
			if (cam.target != "") {

				//get info from target
				int targetId = ECS.getEntity(cam.target);
				Transform& transTarget = ECS.getComponentFromEntity<Transform>(targetId);

				//get camera position
				Transform& transCam = ECS.getComponentFromEntity<Transform>(owner);

				cam.lookAt(transCam.position(), transTarget.position());
			}

			//mark spline as over when ratio resets
			if (ratio >= 1) over = true;
		}
	}
}

void ViewTrack::setInitPos() {
