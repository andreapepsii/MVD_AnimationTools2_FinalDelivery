	...
	if (json.HasMember("cameras")) {
		for (rapidjson::SizeType i = 0; i < json["cameras"].Size(); i++) {
            ...
            if (json["cameras"][i].HasMember("target"))
            {
				...
				std::string the_target = tg.GetString();

				int target_id = ECS.getEntity(the_target);
				Transform& target_tran = ECS.getComponentFromEntity<Transform>(target_id);
				lm::vec3 target_pos = target_tran.position();

				player_cam.forward = (target_pos - the_position).normalize();
