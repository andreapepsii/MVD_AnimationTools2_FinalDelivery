...
bool CameraSystem::lateInit()
{
	...
	//get all tracks in scene
	tracks = ECS.getAllComponents<ViewTrack>();
	currTrack = 0;
	cameraId = ECS.getEntity("Star");
...
void CameraSystem::updateTest(float dt)
{
	...
	tracks = ECS.getAllComponents<ViewTrack>();
	if (tracks.size() > 0) {
		//if the track has ended and the trackTest mode is on,
		//the track is changed to another camera to keep it moving
		if (tracks[currTrack].over && trackTest) {
			std::cout << "test" << std::endl;
			//save current track to initialize it to be replayed
			int currTrackAux = currTrack;

			//change track
			currTrack++;
			if (currTrack >= tracks.size()) {
				currTrack = 0;
			}

			//set new track
			auto& track = ECS.entities[tracks[currTrack].owner];
			SetDefaultCamera(track);
			//activate new track
			ViewTrack& curr = ECS.getComponentFromEntity<ViewTrack>(tracks[currTrack].owner);
			curr.active = true;

			//set old track to the beginning so it be replayed
			ViewTrack& prevTrack = ECS.getComponentFromEntity<ViewTrack>(tracks[currTrackAux].owner);
			prevTrack.setInitPos();
			prevTrack.active = false;

		}


		if (Game::instance->control_system_.input[GLFW_KEY_6] && !trackTest) {
			//grab current track
			ViewTrack& curr = ECS.getComponentFromEntity<ViewTrack>(tracks[currTrack].owner);
			curr.active = true;
			isOver = false;

			Entity& track = ECS.entities[tracks[currTrack].owner];
			SetDefaultCamera(track);
		...
		if (!trackTest) // We are testing the track, nothing else permited.
		{
			if (Game::instance->control_system_.input[GLFW_KEY_0]) {
				isOver = false;
				cameraId = ECS.getEntity("camera_test1");
				Entity& main_camera = ECS.entities[cameraId];
			...
			if (Game::instance->control_system_.input[GLFW_KEY_1]) {
				isOver = false;
				cameraId = ECS.getEntity("camera_test2");
				Entity& main_camera = ECS.entities[cameraId];
				blendInCamera(main_camera, 2.f, CameraSystem::EPriority::GAMEPLAY);
				std::cout << "blending camera" << std::endl;
			}
			...
			if (Game::instance->control_system_.input[GLFW_KEY_2]) {
				isOver = false;
				cameraId = ECS.getEntity("camera_test3");
				Entity& main_camera = ECS.entities[cameraId];
				blendInCamera(main_camera, 2.f, CameraSystem::EPriority::GAMEPLAY);
				std::cout << "blending camera" << std::endl;
			}
			...
			if (Game::instance->control_system_.input[GLFW_KEY_3]) {
				isOver = false;
				cameraId = ECS.getEntity("camera_test4");
				Entity& main_camera = ECS.entities[cameraId];
				blendInCamera(main_camera, 2.f, CameraSystem::EPriority::GAMEPLAY);
				std::cout << "blending camera" << std::endl;
			}
...
void CameraSystem::lookTarget() {

	Camera& cam = ECS.getComponentFromEntity<Camera>(cameraId);
	int default_id = ECS.getEntity(_defaultCamera.name);
	Camera& defaut_cam = ECS.getComponentFromEntity<Camera>(default_id);

	//cojemos la posicion de la camara
	Transform& trans_cam = ECS.getComponentFromEntity<Transform>(cam.owner);
	Transform& trans_default = ECS.getComponentFromEntity<Transform>(default_id);

	//cogemos la id del target
	int target_id = ECS.getEntity(cam.target);

	//cogemos la posicion del target
	Transform& target_tran = ECS.getComponentFromEntity<Transform>(target_id);

	//hacemos el look at al target
	defaut_cam.lookAt(trans_default.position(), target_tran.position());
	cam.lookAt(trans_cam.position(), target_tran.position());
